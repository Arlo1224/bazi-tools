...
const selectedLiuNian = ref<string | null>(null)  // 干支
...
watch(province, () => { const cs = getCities(province.value); if (cs.length > 0) city.value = cs[0].name })

onMounted(async () => {
  await loadCities()
  if (provinceList.value.includes('安徽省')) { province.value = '安徽省'; city.value = '芜湖' }
  try { archives.value = JSON.parse(localStorage.getItem('bazi_archive') || '[]') } catch { archives.value = [] }
})

// ── 五行/十神工具 ──
const ganWuxingMap: Record<string, string> = { '甲':'木','乙':'木','丙':'火','丁':'火','戊':'土','己':'土','庚':'金','辛':'金','壬':'水','癸':'水' }
const zhiWuxingMap: Record<string, string> = { '子':'水','丑':'土','寅':'木','卯':'木','辰':'土','巳':'火','午':'火','未':'土','申':'金','酉':'金','戌':'土','亥':'水' }
const ganYinYang: Record<string, string> = { '甲':'阳','乙':'阴','丙':'阳','丁':'阴','戊':'阳','己':'阴','庚':'阳','辛':'阴','壬':'阳','癸':'阴' }
function wuxingClass(w: string) { const m: Record<string, string> = { '木':'wuxing-wood','火':'wuxing-fire','土':'wuxing-earth','金':'wuxing-metal','水':'wuxing-water' }; return m[w]||'' }
function charWuxingClass(c: string) { return wuxingClass(ganWuxingMap[c] || zhiWuxingMap[c] || '') }
...
// ── 干支关系 ──
const pillarRelations = computed(() => {
  const r: string[] = []
  const xch = result.value?.刑冲合会
  if (!xch) return r
  const names: Record<string, string> = { '年':'年柱','月':'月柱','日':'日柱','时':'时柱' }
  for (const pillar of ['年','月','日','时']) {
    const pd = xch[pillar]
    if (!pd) continue
    // 天干关系
    const tg = pd.天干 || {}
    for (const rel of ['合','冲','刑','害'] as const) {
      for (const item of (tg[rel] || [])) {
        r.push(`${names[pillar]}天干${names[pillar]}为${rel === '合' ? '合' : rel === '冲' ? '冲' : rel === '刑' ? '刑' : '害'}于${names[item.柱]}天干（${item.知识点}）`)
      }
    }
    // 地支关系
    const dz = pd.地支 || {}
    for (const rel of ['合','冲','刑','害','破']) {
      for (const item of (dz[rel] || [])) {
        r.push(`${names[pillar]}地支${names[pillar]}为${rel}于${names[item.柱]}地支（${item.知识点}）`)
      }
    }
  }
  return r
...
const isSixPillar = computed(() => extraPillars.value.length > 0)

// ── 排盘 ──
function doPaipan() {
  error.value = ''
  selectedDayunIdx.value = null; selectedLiuNian.value = null; selectedLiuNianYear.value = null
  try {
    const cityInfo = findCity(city.value); const longitude = cityInfo?.gpsLon ?? 120
    ...
    if (useTrueSolar.value) { const tr = calcTrueSolarTime(inputTime, longitude); solarTimeStr = formatTime(tr.result); correction = tr.correctionMinutes }
    else { solarTimeStr = formatTime(inputTime) }
    const baziResult = buildBaziFromSolar({ solarTime: solarTimeStr, sect: 2, gender: gender.value })
    result.value = { ...baziResult, _meta: { inputTime: formatTime(inputTime), trueSolarTime: solarTimeStr, correction, city: city.value, longitude, useTrueSolar: useTrueSolar.value } }
    emit('update:result', result.value)
    const entry = {
      id: Date.now(), time: new Date().toLocaleString('zh-CN'), name: personName.value || '未命名',
      bazi: baziResult.八字, dayMaster: baziResult.日主, solar: baziResult.阳历, lunar: baziResult.农历, gender: baziResult.性别,
      meta: result.value._meta, result: JSON.parse(JSON.stringify(baziResult)),
    }
    archives.value.unshift(entry); if (archives.value.length > 50) archives.value = archives.value.slice(0, 50)
...
function selectDayun(i: number) { selectedDayunIdx.value = selectedDayunIdx.value === i ? null : i }
function selectFortuneItem(item: any) {
  if (fortuneLevel.value === 'year') { selectedLiuNian.value = item.gz; selectedLiuNianYear.value = item.year; fortuneLevel.value = 'year' }
  else if (fortuneLevel.value === 'month') { selectedLiuNian.value = null; selectedLiuNianYear.value = null }
  else if (fortuneLevel.value === 'day') { selectedLiuNian.value = null; selectedLiuNianYear.value = null }
...
const fortuneResults = computed(() => {
  ...
  if (fortuneLevel.value === 'year') {
    for (let y = fStartYear.value; y < fStartYear.value + fRangeYears.value; y++) {
      try { const sd = SolarDay.fromYmd(y, 6, 1); const sc = sd.getLunarDay().getLunarMonth().getLunarYear().getSixtyCycle(); const gz = sc.getName(); results.push({ label: `${y}年`, year: y, gz, s0: getShiShen(dayMaster.value, gz[0]) }) } catch {}
    }
  } else if (fortuneLevel.value === 'month') {
    for (let m = 1; m <= 12; m++) {
      try { const sd = SolarDay.fromYmd(fStartYear.value, m, 15); const sc = sd.getLunarDay().getLunarMonth().getSixtyCycle(); const gz = sc.getName(); results.push({ label: `${m}月`, gz, s0: getShiShen(dayMaster.value, gz[0]) }) } catch {}
    }
  } else if (fortuneLevel.value === 'day') {
    for (let d = 1; d <= fMaxDays.value; d++) {
      try { const sd = SolarDay.fromYmd(fStartYear.value, fMonth.value, d); const sc = sd.getLunarDay().getSixtyCycle(); const gz = sc.getName(); results.push({ label: `${d}日`, gz, s0: getShiShen(dayMaster.value, gz[0]) }) } catch {}
    }
  } else {
    const scNames = ['子','丑','寅','卯','辰','巳','午','未','申','酉','戌','亥']
    for (let h = 0; h < 24; h += 2) {
      try { const lh = LunarHour.fromYmdHms(fStartYear.value, fMonth.value, fDay.value, h, 0, 0); const sc = lh.getSixtyCycle(); const gz = sc.getName(); results.push({ label: `${scNames[h / 2]}时`, sub: `${String(h).padStart(2,'0')}:00-${String(h+1).padStart(2,'0')}:59`, gz, s0: getShiShen(dayMaster.value, gz[0]) }) } catch {}
    }
  }
  return results
})
...
<template>
  <div class="bazi-page">
    ...
    <div class="card form-card">
      ...
      <div class="form-row">
        <div class="form-group"><label>姓名</label><input class="form-input" v-model="personName" placeholder="选填" /></div>
      </div>
      <div class="form-row">
    ...
    <div v-if="result" class="card result-card">
      ...
      <div class="bazi-display">
        ...
        <span v-if="isSixPillar" class="six-pillar-text">
          {{ selectedDayunIdx != null ? result.大运.大运[selectedDayunIdx]?.干支 : '' }} {{ selectedLiuNian }}
        </span>
            ...
            <tr>
              ...
              <td v-for="key in ['年柱','月柱','日柱','时柱']" :key="key">
                <div class="canggan">
                  ...
                  <span v-if="result[key]?.地支?.藏干?.中气" :class="charWuxingClass(result[key].地支.藏干.中气.天干)">{{ result[key].地支.藏干.中气.天干 }}<small>{{ result[key].地支.藏干.中气.十神 }}</small></span>
      ...
      <div class="extra-info">
        <span>胎元：{{ result.胎元 }}</span><span>命宫：{{ result.命宫 }}</span><span>身宫：{{ result.身宫 }}</span>
      </div>

      <!-- 干支关系 -->
      <div v-if="pillarRelations.length > 0" class="section-block">
        <h4>干支关系</h4>
        <div class="rel-list">
          <div v-for="(rel, i) in pillarRelations" :key="i" class="rel-item">{{ rel }}</div>
        </div>
      </div>
      ...
      <div v-if="result.神煞" class="section-block">
        ...
        <div class="shensha-grid">
          <div v-for="key in ['年柱','月柱','日柱','时柱']" :key="key"><span class="shensha-label">{{ key }}：</span><span>{{ result.神煞[key]?.join('、') || '无' }}</span></div>
        </div>
      </div>

      <!-- 大运 -->
      <div v-if="result.大运" class="section-block">
        <h4>大运 <small>（{{ result.大运.起运年龄 }}岁起运 · {{ result.大运.起运日期 }}）点选大运+流年可展开六柱</small></h4>
        <div class="dayun-scroll">
          <div v-for="(dy, i) in result.大运.大运?.slice(0, 8)" :key="i"
               class="dayun-item" :class="{ 'dayun-selected': selectedDayunIdx === Number(i) }"
               @click="selectDayun(Number(i))">
      ...
      <div class="section-block">
        ...
        <div class="fortune-tabs">
          <button v-for="l in (['year','month','day','hour'] as const)" :key="l" :class="{ active: fortuneLevel === l }" @click="fortuneLevel = l">{{ {year:'流年',month:'流月',day:'流日',hour:'流时'}[l] }}</button>
        </div>
        <div class="fortune-params">
          <template v-if="fortuneLevel === 'year'">
            <select class="form-input" v-model.number="fStartYear" style="max-width:120px"><option v-for="y in Array.from({length:50},(_,i)=>2000+i)" :key="y" :value="y">{{ y }}年</option></select>
            <select class="form-input" v-model.number="fRangeYears" style="max-width:100px"><option :value="5">5年</option><option :value="10">10年</option><option :value="20">20年</option></select>
          </template>
          ...
          <template v-else>
            <select class="form-input" v-model.number="fStartYear" style="max-width:120px"><option v-for="y in Array.from({length:50},(_,i)=>2000+i)" :key="y" :value="y">{{ y }}年</option></select>
            <select class="form-input" v-model.number="fMonth" style="max-width:100px"><option v-for="m in 12" :key="m" :value="m">{{ m }}月</option></select>
            <select v-if="fortuneLevel === 'hour'" class="form-input" v-model.number="fDay" style="max-width:100px"><option v-for="d in fMaxDays" :key="d" :value="d">{{ d }}日</option></select>
          </template>
        ...
        <div class="fortune-grid" :class="{ 'fortune-grid-dense': fortuneLevel === 'day' }">
          <div v-for="(item, i) in fortuneResults" :key="i"
               class="fortune-item" :class="{ 'fortune-item-sm': fortuneLevel === 'day', 'fortune-selected': fortuneLevel === 'year' && selectedLiuNian === item.gz }"
               @click="selectFortuneItem(item)">
...
.form-row-end { align-items: center; gap: 12px; margin-top: 4px; }
...
.archive-item { display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: var(--card); border: 1px solid var(--border); border-radius: var(--radius-sm); cursor: pointer; transition: border-color 0.2s; }
...
.bazi-display { display: flex; align-items: baseline; gap: 12px; flex-wrap: wrap; margin-bottom: 16px; }
.bazi-text { font-size: 22px; font-weight: 700; letter-spacing: 4px; color: var(--accent); }
.six-pillar-text { font-size: 16px; font-weight: 600; color: var(--text); letter-spacing: 2px; }
...
.th-extra { background: var(--accent-dim); color: #e0a060; }
.td-extra { background: #1f1f18 !important; }
...
.section-block h4 small { font-size: 11px; color: var(--text-muted); font-weight: 400; }

/* 干支关系 */
.rel-list { font-size: 12px; }
.rel-item { padding: 4px 8px; margin-bottom: 3px; background: var(--bg); border-radius: var(--radius-sm); color: var(--text-dim); }
...
.dayun-item { flex-shrink: 0; display: flex; flex-direction: column; align-items: center; gap: 2px; padding: 8px 14px; background: var(--bg); border: 1px solid var(--border); border-radius: var(--radius-sm); font-size: 12px; min-width: 70px; cursor: pointer; transition: all 0.2s; }
.dayun-item:hover { border-color: var(--accent); }
.dayun-selected { border-color: var(--accent); background: var(--accent-dim); box-shadow: 0 0 8px rgba(201,169,110,0.2); }
...
.fortune-tabs button { flex: 1; padding: 6px; border: 1px solid var(--border); background: var(--bg); color: var(--text-dim); border-radius: var(--radius-sm); cursor: pointer; font-family: inherit; font-size: 12px; transition: all 0.2s; }
.fortune-tabs button.active { background: var(--accent-dim); color: var(--accent); border-color: var(--accent-dim); }
.fortune-params { display: flex; gap: 8px; align-items: center; margin-bottom: 12px; flex-wrap: wrap; }
.liunian-tag { font-size: 11px; color: var(--accent); background: var(--accent-dim); padding: 4px 8px; border-radius: 4px; }
...
.fortune-item { display: flex; flex-direction: column; align-items: center; gap: 1px; padding: 8px 4px; background: var(--bg); border: 1px solid var(--border); border-radius: var(--radius-sm); text-align: center; cursor: pointer; transition: all 0.15s; }
.fortune-item:hover { border-color: var(--accent); }
.fortune-selected { border-color: var(--accent); background: var(--accent-dim); }
